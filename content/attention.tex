...
Peripheral displays are designed to just enable passive monitoring of information. In critical moments, however, the display has to capture the user's attention, which can be achieved by using motion.
Still, some information is more important than other, therefore, notifications should be distiguished into levels, corresponding to that priority.
...
\textcite{matthewsToolkitManagingUser2004} present a "Peripheral Displays Toolkit", which helps to assign notification levels to information and shows how these transitions between those levels should look.
% attention levels in HVS?
First, they categorize attention into inattention, divided attention and focused attention. These categories can then be matched to notification levels. Transitions serve to create effects that capture an appropiate amount of attention.
Abrupt transitions, for example, are meant to interrupt the user to attract focal attention. When aiming for divided attention, abruptly updating smaller areas of the screen or using slow movements should be sufficient.
